...
tab <- table(df$stack_907, df$D5_rec) %>% as.data.frame()
names(tab)[1:2] <- c('stack_907', 'D5_rec')


# All the contingency tables # 
#Model 6: (list entry 4)
tabs6 <- yxcontab.auxfun(regdf_lst$logit[[6]], contab = F)
#Model 7: (list entries 3,4, and 5)
tabs7 <- yxcontab.auxfun(regdf_lst$logit[[7]], contab = F)

#huge SE in above mentioned variables of models 6 and 7 most likely due to 
...
#Model 6: Except for Model 6 the null hypothesis cannot be rejected. 
#Unconstrained model 6 fits better than the constrained one at p<0.1. This does 
#not meet the required p<0.05. Thus the constrained model will not be implemented 
#for Model 6.   

#Model 7: Except for Model 7 the null hypothesis cannot be rejected. 
#Unconstrained model 7 fits better than the constrained one at p<0.05. This meets 
#the required p<0.05. Thus the constrained model will be implemented for Model 7. 
