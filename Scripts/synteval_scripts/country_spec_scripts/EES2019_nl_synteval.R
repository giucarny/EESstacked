# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Title: Script for Evaluating Synthetic Variables Estimation (EES 2019 Voter Study, Netherlands Sample) 
# Author: w.Haeussling
...
EES2019_nl <- EES2019 %>% filter(countryshort==cntry)
EES2019_stckd_nl <- EES2019_stckd %>% filter(countryshort==cntry)
EES2019_cdbk_nl <- EES2019_cdbk %>% filter(countryshort==cntry)
...
EES2019_nl_stack <- 
  cbind(EES2019_stckd_nl,  
        lapply(data = EES2019_stckd_nl, 
...
EES2019_nl_stack %<>%
  cbind(.,
        lapply(data = EES2019_nl,
               cdbk = EES2019_cdbk_nl,
               stack = EES2019_nl_stack, 
               crit = 'average',
               rescale = T,
               check = F,
               keep_id = F,
...
csdf_lst <- list('std'  = EES2019_nl,
                 'cdbk' = EES2019_cdbk_nl,
                 'SDM'  = EES2019_nl_stack)
...
# No models show any inflated SE on predictors. Therefore, we do not adapt the models.
