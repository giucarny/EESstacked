...
#For the logit model 3 D8_rec, Edu_rec2, EDU_rec3, D1_rec, D7_rec2 and the 
#constant are affected.
#For the logit model 4 D8_rec, D7_rec1, D7_rec2 and the constant are affected.
#For the logit model 5 D8_rec, EDU_rec3, D7_rec2 and the constant are affected.

# One contingency table # 

df <- regdf_lst$logit[[3]] 
tab <- table(df$stack_1903, df$D8_rec) %>% as.data.frame()
names(tab)[1:2] <- c('stack_1903', 'D8_rec')


# All the contingency tables # 
#Model 3: 
tabs3 <- yxcontab.auxfun(regdf_lst$logit[[3]], contab = F)
#Model 4: 
tabs4 <- yxcontab.auxfun(regdf_lst$logit[[4]], contab = F)
#Model 5: 
tabs5 <- yxcontab.auxfun(regdf_lst$logit[[5]], contab = F)

#huge SE in above mentioned variables of models 3, 4 and 5 most likely due to 
#empty or nearly empty cells observable in the contingency tables
...
regdf_lst_part3 <- 
  regdf_lst$logit %>% 
  lapply(., function(x){ x %<>% na.omit() %>% dplyr::select(-c(D8_rec, EDU_rec, D1_rec, D7_rec))})
...
regdf_lst_part4 <- 
  regdf_lst$logit %>% 
  lapply(., function(x){ x %<>% na.omit() %>% dplyr::select(-c(D8_rec, D7_rec))})
...
regdf_lst_part5 <- 
  regdf_lst$logit %>% 
  lapply(., function(x){ x %<>% na.omit() %>% dplyr::select(-c(D8_rec, EDU_rec, D7_rec))})
