...
EES2019_fr <- EES2019 %>% filter(countryshort==cntry)
EES2019_stckd_fr <- EES2019_stckd %>% filter(countryshort==cntry)
EES2019_cdbk_fr <- EES2019_cdbk %>% filter(countryshort==cntry)
...
EES2019_fr_stack <- 
  cbind(EES2019_stckd_fr,  
        lapply(data = EES2019_stckd_fr, 
...
EES2019_fr_stack %<>%
  cbind(.,
        lapply(data = EES2019_fr,
               cdbk = EES2019_cdbk_fr,
               stack = EES2019_fr_stack, 
               crit = 'average',
               rescale = T,
               check = F,
               keep_id = F,
...
csdf_lst <- list('std'  = EES2019_fr,
                 'cdbk' = EES2019_cdbk_fr,
                 'SDM'  = EES2019_fr_stack)
...
#no problems occur in logit or OLS regression, thus no partial model modification necessary
