...
In terms of model fit, the coefficient of determination ($R^2$) values vary between 
a minimum value of `r ols_df %>% filter(model=='full') %>% filter(Rsq==min(Rsq)) %>% .[['Rsq']]` 
for party `r ols_df %>% filter(model=='full') %>% filter(Rsq==min(Rsq)) %>% .[['partycode']]` 
(`r ols_df %>% filter(model=='full') %>% filter(Rsq==min(Rsq)) %>% .[['partyname_eng']]`) 
and a maximum of `r ols_df %>% filter(model=='full') %>% filter(Rsq==max(Rsq)) %>% .[['Rsq']]`
for party `r ols_df %>% filter(model=='full') %>% filter(Rsq==max(Rsq)) %>% .[['partycode']]`
(`r ols_df %>% filter(model=='full') %>% filter(Rsq==max(Rsq)) %>% .[['partyname_eng']]`). 
...
* Model 5a: D8_rec, D5_rec, EDU_rec, D7_rec (only for category 2), D6_une;
* Model 6: D6_une.

Nevertheless, models 3 and 6 constant terms and other regression coefficients are not affected by said 
...
As a consequence, a constrained model 5 (namely, Model 5b) without said variables has been estimated and 
contrasted with the original (Model 5a), full model. Likelihood-ratio test results show that $H_0$ 
(namely, that the constrained model fits better than the full model) cannot be rejected (see Table 
\ref{table:lrtest_1_cy}). Consequently, synthetic variables for respondents' vote choice for party 505 
have been predicted relying on the constrained model. 
...
logit_regtab[9] %<>%
  gsub('Model 5', 'Model 5a',.) %>% 
  gsub('Model 6', 'Model 5b',.) %>% 
  gsub('Model 7', 'Model 6',.) 
