...
## socdem_synt_vc {-}
Synthetic variable, measuring the affinity between the respondents' socio-demographic characteristics and their vote choice. This was estimated using a linear prediction of a logit model. The dependent Variable of this regression analysis was `Q10_gen` (see: Generic distance/proximity variables estimation) and the independent variables `D3_rec`, `D5_rec`, `D8_rec`, `EDU_rec`, `D4_age` and `D10_rec` (see below).
