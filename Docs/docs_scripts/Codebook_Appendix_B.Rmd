...
(see Table \ref{table:relprty_tab_at}). 
...
logit_aic %>% 
  kable(caption = "Akaike Information Criterion values for logistic full and null models 
        \\label{table:logit_aic_at}", booktabs = T, 
        align = c('l', 'c', rep('r',3))) %>% 
  kable_styling(latex_options = c('striped', 'hold_position')) %>% 
...
(see Tables \ref{table:relprty_tab_be_du} and \ref{table:relprty_tab_be_fr}).
...
Synthetic variables have been estimated for the full set of relevant parties available in the original 
2019 EES Bulgarian voter study (see Table \ref{table:relprty_tab_bg}). 
...
tabs[[5]] %>% 
  kable(caption = "Cross tabulation between vote choice for party 307 and respondents' subjective social class
                   \\label{table:crosstab_5_bg}", booktabs = F) %>% 
  kable_styling(latex_options = c('striped'))
...
(see Table \ref{table:relprty_tab_cz}).
...
(see Table \ref{table:relprty_tab_hr}). 
...
(see Table \ref{table:relprty_tab_cy}). 
...
(see Table \ref{table:relprty_tab_dk}). 
...
Synthetic variables have been estimated for seven of twelve Estonian parties available in the original 
2019 EES Estonian voter study selected according to the criteria stated in the EES 2019 SDM codebook 
(see Table \ref{table:relprty_tab_ee}). 
...
anova_lst7_1[[1]] %>%
  kable(caption = "Likelihood-ratio Test between Model 14a (Unconstrained) and Model 14b1 (Constrained and without D5rec) \\label{table:lrtest_2_ee}", booktabs = F, ) %>%
  kable_styling(latex_options = c('striped', 'hold_position'))
...
anova_lst7_2[[1]] %>%
  kable(caption = "Likelihood-ratio Test between Model 14a (Unconstrained) and Model 14b2 (Constrained and with D5rec) \\label{table:lrtest_3_ee}", booktabs = F, ) %>%
  kable_styling(latex_options = c('striped', 'hold_position'))
...
anova_lst7_1vs2[[1]] %>%
  kable(caption = "Likelihood-ratio Test between Model 14b1 (Constrained and without D5rec, here Constrained) and Model 14b2 (Constrained and with D5rec, here Unconstrained) \\label{table:lrtest_4_ee}", booktabs = F) %>%
  kable_styling(latex_options = c('striped', 'hold_position'))
...
(see Table \ref{table:relprty_tab_fi}).
...
(see Table \ref{table:relprty_tab_fr}). 
...
(see Table \ref{table:relprty_tab_de}). 
...
(see Table \ref{table:relprty_tab_el}).
...
(see Table \ref{table:relprty_tab_hu}).
...
(see Table \ref{table:relprty_tab_ie}). 
...
(see Table \ref{table:relprty_tab_it}). 
...
(see Table \ref{table:relprty_tab_lv}). 
...
On the contrary, six out of seven logistic regression models (see Table \ref{table:full_logit_lv_p1}) 
show inflated standard errors for some of the coefficients of interest, 
in particular: 
...
logit_regtab[9] %<>%
  gsub('Model 10', 'Model 14',.) %>%
  gsub('Model 9', 'Model 13b',.) %>%
  gsub('Model 3', 'Model 9',.) %>%
  gsub('Model 4', 'Model 10',.) %>%
  gsub('Model 5', 'Model 11',.) %>%
  gsub('Model 6', 'Model 12a',.) %>%
  gsub('Model 7', 'Model 12b',.) %>%
  sub('Model 8', 'Model 13a',.) %>%
  sub('Model 2', 'Model 8b',.) %>%
  sub('Model 1', 'Model 8a',.)
...
(see Table \ref{table:relprty_tab_lt}).
...
(see Table \ref{table:relprty_tab_lu}). 
...
(see Table \ref{table:relprty_tab_mt}). 
...
Model 8a inflated standard errors are due to separation issues. In short, no respondents from rural areas, with low education, with high subjective social status, members of trade unions, and unemployed did vote for party 1903 (see Tables \ref{table:crosstab_3_1_mt}, \ref{table:crosstab_3_2_mt}, \ref{table:crosstab_3_3_mt}, \ref{table:crosstab_3_4_mt}, \ref{table:crosstab_3_5_mt}).
...
(see Table \ref{table:relprty_tab_nl}). 
...
(see Table \ref{table:relprty_tab_pl}).
...
2019 EES Romanian voter study selected according to the criteria stated in the EES 2019 SDM codebook 
(see Table \ref{table:relprty_tab_ro}). 
...
(see Table \ref{table:relprty_tab_sk}).
...
(see Table \ref{table:relprty_tab_si}). 
...
(see Table \ref{table:relprty_tab_es}). 
...
(see Table \ref{table:relprty_tab_se}).
...
(see Table \ref{table:relprty_tab_uk}). 
...
>>>>>>> 1e0662c899ddb7637ec815125b5adedc65561a74
