...
Then, you can develop an additional paragraph just below the one dedicated to the dichotomous 
variables^[Actually you could also put the `gendis.fun` in the same chunk of code used for estimating 
the dichotomous variables with the `gendic.fun`. For sake of clarity, I personally prefer to keep the 
...
Of course, we could add more variables to the chunk above, just copy-pasting the `gendic.fun` function 
and changing the argument `var`, or we could apply such function to a vector containing the 
variables of interest, then binding the results, and merging the output with the SDM. 
The latter is the solution that you will actually find in the exemplary script, as shown just below this 
paragraph

```{r, eval=T, echo=F}

EES2019_it_stack %<>% dplyr::select(-c(Q10_gen)) 
